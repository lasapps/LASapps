...
*/
  ...
  while (lasreadopener.active())
  {
	...
	*/
	...
	if (poDS == NULL)
	{
		fprintf(stderr, "Open shapefile failed.\n");
		byebye(true, argc == 1);
	}
	...
	*/
	...
	while (lasreader->read_point() && ((poFeature = poLayer->GetNextFeature()) != NULL))
	{
		...
		*/
		...
		{
			fprintf(stderr, "no point geometry\n");
		}
...
#ifdef _WIN32
    if (verbose) fprintf(stderr,"comparing %I64d points of '%s' took %g sec.\n", lasreader->p_count, lasreadopener.get_file_name(), taketime()-start_time);
#else
    if (verbose) fprintf(stderr,"comparing %lld points of '%s' took %g sec.\n", lasreader->p_count, lasreadopener.get_file_name(), taketime()-start_time);
#endif
